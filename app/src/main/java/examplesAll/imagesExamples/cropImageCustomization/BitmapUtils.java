...
final class BitmapUtils {
  ...
  static Bitmap resizeBitmap(
          Bitmap bitmap, int reqWidth, int reqHeight, CropImageVieww.RequestSizeOptions options) {
    try {
      if (reqWidth > 0
          && reqHeight > 0
          && (options == CropImageVieww.RequestSizeOptions.RESIZE_FIT
              || options == CropImageVieww.RequestSizeOptions.RESIZE_INSIDE
              || options == CropImageVieww.RequestSizeOptions.RESIZE_EXACT)) {

        Bitmap resized = null;
        if (options == CropImageVieww.RequestSizeOptions.RESIZE_EXACT) {
          resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
        } else {
          int width = bitmap.getWidth();
          int height = bitmap.getHeight();
          float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
          if (scale > 1 || options == CropImageVieww.RequestSizeOptions.RESIZE_FIT) {
            resized =
